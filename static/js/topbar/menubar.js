...
menubar_module.menu_content_wrapper_class = '.menubar_item_content_wrapper'
...
menubar_module.vars.is_opened = false;
...
//если клик вне menubar скрыть меню
menubar_module.body.on('click', function(e){
	if (menubar_module.mobile.screen == false){
		if (e.target == menubar_module.menu_content_wrapper 
			|| $(e.target).parents(menubar_module.menu_content_wrapper_class).size()){

			console.log('inside menu');
		} else {
			menubar_module.menu_content_wrapper.hide();
			menubar_module.triangle.hide();
		}
	}
});
...
menubar_module.mobile.open.on('click', function(){
	menubar_module.vars.is_opened = true;
